 ...
 */
export function padLeft(str, chars, sign) {
 ...
 * Should be called to pad string to expected length
 */
export function padRight(str, chars, sign) {
 ...
 */
export function fromAscii(str, num = 0) {
    let hex = '';
 ...
 */
export function extractTypeName(name) {
    let stBracket = name.indexOf('(');
    let endBracket = name.indexOf(')');
 ...
 * Converts value to it's decimal representation in string
 */
export function toNullDecimal(value) {
 ...
 */
export function toStringData(val) {
    if (typeof val === 'string') {
        if (val.startsWith('0x') && /^[A-Za-z0-9]+$/.test(val)) {
            return toHex(val);
        }
...
export function toHex(val) {
    ...
    if (isString(val)) {
        ...
        if (valStr.indexOf('-0x') === 0)
            ...
            return bytesToHex(stringToUtf8Bytes(valStr));
    }
    if (val instanceof Uint8Array)
        return '0x' + bytesToHex(val);
 ...
 */
export function getValueOfUnit(_unit) {
 ...
 */
export function toBigNumber(_num) {
    let num = _num || 0;
    if (isBigNumber(num)) {
        return num;
    }
 ...
 */
export function toTwosComplement(num, bits = 256) {
 ...
 */
export function signedIsNegative(value, bits) {
 ...
 * If the bit N is 1
 */
export function fromTwosComplement(num, bits = 256) {
 ...
 */
export function isChecksumAddress(_address) {
    // Check each case
